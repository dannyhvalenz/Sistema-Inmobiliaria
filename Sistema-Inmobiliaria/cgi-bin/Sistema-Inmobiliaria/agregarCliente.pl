...
my $sth = $dbh->prepare("INSERT into Cliente (nombre, apellidoP, apellidoM, correo, celular, idAsesor) values (?,?,?,?,?,?)");
$sth->execute($nombre, $apellidoP, $apellidoM, $correo, $celular, $idAsesor) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo cliente');
    $writer->dataElement('contenido', 'Se ha registrado el cliente');
} else {
    $writer->dataElement('titulo', 'Error de conexion');
    $writer->dataElement('contenido', 'No se ha podido registrar el cliente');
}
