...
my $sth = $dbh->prepare("UPDATE Propietario SET nombre=?,apellidoP=?,apellidoM=?,correo=?,celular=?,direccion=? WHERE idPropietario=?");
$sth->execute($nombre,$apellidoP,$apellidoM,$correo,$celular,$direccion,$idPropietario) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Propietario actualizado');
    $writer->dataElement('contenido', 'La informacion del propietario fue actualizada');
} else {
    $writer->dataElement('titulo', 'Error de conexion');
    $writer->dataElement('contenido', 'No se pudo actualizar la informacion del propietario');
}
